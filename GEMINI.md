...
- **INFO title (touchstart/touchend split):** `touchstart` → `ApexSound.init()` only (ctx.resume() fires within gesture). `touchend` → `startWhistle()` — by then ctx.resume() has resolved. `click` kept for desktop. NOTE: This split was later found unreliable in practice — replaced March 24 Session 2 with `click → startWhistle()` only (init + startLoop in same gesture).
...
## Progress Log (March 24, 2026) — iOS Safari Sound: Site-Wide Regression Fix

### Root Cause
Gemini's 8 commits to `apex-sound.js` during the March 24 Session 1 broke the audio engine. The final "restore" commit (`ae48e61`) was missing the critical `.catch()` on `ctx.resume()`:

```js
// Gemini's broken version:
_resumePromise = ctx.state === 'suspended' ? ctx.resume() : Promise.resolve();

// Correct version:
_resumePromise = ctx.state === 'suspended' ? ctx.resume().catch(function () {}) : Promise.resolve();
```

Without `.catch()`, a rejected resume promise makes `_resumePromise` a rejected promise. `rp.then(_dispatch)` chains on it silently, fires nothing. The audio engine appeared to work (menu open plays door sound) but only because the menu's gesture handler called `init()` + `play()` in the same synchronous gesture — bypassing the broken `_resumePromise` chain.

### Fix 1 — Engine Restore (`ada5d13`)
`apex-sound.js` restored exactly to `eb9da01` via `git show eb9da01:src/apex-sound.js > src/apex-sound.js`. Both `.catch(function() {})` calls reinstated. `play()`/`startLoop()` restored to NOT call `init()` themselves (they return early if `!ctx`) — `init()` must always come from a gesture.

### Fix 2 — Choice Cuts + Long Pour Trigger (`8298e7c`)
`document.addEventListener('touchend', slice, { once: true })` removed from both pages. This fired on ANY first touchend — including menu button taps — consuming the `sliced`/`poured` flag before the user had scrolled. Sound never played.
...
function slice() {
    if (sliced) return;
    sliced = true;
    ApexSound.init();       // ← init AND play in same function
    ApexSound.play('knife', 0.8);
}
document.addEventListener('touchstart', function() {
    ApexSound.init();
}, { passive: true, once: true });  // pre-warm only; no flag consumed
window.addEventListener('scroll', function onFirstScroll() {
    if (window.scrollY > 50) { slice(); ... }
}, { passive: true });
```

### Fix 3 — INFO Title Click (`f8fe27e`)
Touchstart/touchend split on `infoTitle` removed. Was:
```js
infoTitle.addEventListener('touchstart', function() { ApexSound.init(); }, { passive: true });
infoTitle.addEventListener('touchend', startWhistle, { passive: true });
infoTitle.addEventListener('click', startWhistle);
```
If iOS doesn't honor touchstart as an audio-unlock gesture, `_resumePromise` hangs unresolved. `startWhistle()` fires on touchend before resume settles → silence.

Replaced with:
```js
infoTitle.addEventListener('click', startWhistle);
// startWhistle() calls ApexSound.init() + ApexSound.startLoop() — init and play in same gesture ✓
```
`click` fires after touchend on iOS and IS honored as a user gesture. Same pattern as the menu button.

### Cache Bust — `?v=2`
Added `?v=2` to `apex-sound.js` script src on all pages (index, 404, exhibits, info, choice-cuts, long-pour) to force Safari to bypass its cached broken engine.

### Confirmed Working Patterns (post-fix)
| Trigger type | Pattern |
|---|---|
| Element click | `element.addEventListener('click', fn)` where `fn` calls `init()` + `play()/startLoop()` |
| Scroll (after touch) | `document touchstart {once}` → `init()`, then scroll → `play()/startLoop()` |
| Menu (all pages) | `click → init() + play('door')` in same handler — always worked |

### Anti-Patterns (confirmed broken, do not re-introduce)
- `touchstart → init(); touchend → play()` split: may leave `_resumePromise` hanging
- `document.addEventListener('touchend', fn, { once: true })`: fires on menu taps, not just scroll
- `ctx.resume()` without `.catch()`: rejected promise silently kills dispatch chain
